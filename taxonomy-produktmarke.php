    ...
    <section class="bg-white mb-10 pt-4 lg:pt-8 lg:mb-20">
        <div class="container px-4 mx-auto my-5">
            <?php if ( apply_filters( 'woocommerce_show_page_title', true ) ) : ?>
                <h1 class="text-right text-left text-black-50 text-4xl lg:text-5xl font-medium mb-8 woocommerce-products-header__title page-title"><?php woocommerce_page_title(); ?></h1>
            <?php endif; ?>
...
$ID_brend = get_queried_object()->term_id;
...
?>
    ...
    <section>
        <div class="container px-4 mx-auto my-5">
            <input id="current_tax" type="hidden" value="<?= $ID_brend;?>">
            <h4 class="text-2xl text-black-50 font-medium mb-7"><?php esc_html_e( 'Filtern' ); ?></h4>
            <form action="#" method="post" accept-charset="utf-8" class="text-base lg:text-lg text-black-50 font-light filter_produktmarke" id="filter_produktmarke">
                <div class="lg:flex items-center mb-3 lg:mb-7">
                    <label class="mr-2 mb-2 lg:pointer-events-none category-label">
                        ...
                        <span class="inline-block ml-1 lg:hidden caret" style="position: relative; top:2px;"><svg width="10" height="13" viewBox="0 0 10 13" fill="none"><path d="M10 6.5L0.25 12.1292V0.870835L10 6.5Z" fill="#475D63"/></svg></span>
                    </label>
                    <div class="hidden lg:block category mt-4 lg:mt-0" >
                        ...
                        if ( isset($terms_category) && !empty($terms_category) ) {
                            foreach ($terms_category as $key => $value) {
                                if($value->slug != 'filter-zubehoer'):
                                ?>
                                <label for="<?php echo $value->slug ?>" class="inline-block mx-1 lg:mx-2 mb-2 transform -skew-x-12 <?php echo $value->slug ?>">
                                    <input type="radio"  name="category"  id="<?php echo $value->slug ?>"  value="<?php echo $value->term_id ?>" class="d-none hidden">
                                    <p class="cursor-pointer mb-0 inline-block rounded bg-gray-50 transition duration-500 ease-in-out">
                                        <span class="px-4 py-1 inline-block transform skew-x-12"><?php echo $value->slug ?></span>
                                    </p>
                                </label>
                                <?php
                            endif;
                        ...
                        ?>
                        <label for="all_catefory" class="inline-block mx-1 lg:mx-2 mb-2 transform -skew-x-12">
                ...
                <div class="lg:flex items-center mb-3 lg:mb-7">
                    ...
                    <div class="einsatzgebiet hidden lg:block mt-4 lg:mt-0">
                        ...
                        <label for="all_einsatzgebiet" class="inline-block mx-1 lg:mx-2 mb-2 transform -skew-x-12">
                            ...
                            <p class="cursor-pointer mb-0 inline-block rounded bg-gray-50 transition duration-500 ease-in-out">
                                <span class="px-4 py-1 inline-block transform skew-x-12">Alle</span>
                            </p>
                        </label>
                    </div>
                </div>
                <button type="submit" class="btn-form hidden"> Filtern </button>
            </form>
    ...
    <section class="bg-white mb-0 mt-10 lg:mb-14 lg:mt-16">
        <div class="container px-4 mx-auto my-5">
            ...
            <?php if(!empty($series_category)): ?>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-1 md:gap-4 lg:gap-11 render_res">
                    ...
                    <div class="card mb-7">
                        <div class="mb-8 rounded bg-gray-50 overflow-hidden h-40 max-h-40 md:h-56 md:max-h-56 lg:h-72 lg:max-h-72">
                            <?php if(!empty($term_image)): ?>
                                <a href="/<?=$term_tax?>/<?=$term_slug?>?current=<?=$ID_brend?>"
                        ...
                        <div class="card-body">
                            ...
                            <?php if(!empty($term_link)): ?>
                                <a href="/<?=$term_tax?>/<?=$term_slug?>?current=<?=$ID_brend?>" class="transition duration-500 ease-in-out text-green-50  text-base lg:text-lg font-light text-link">
